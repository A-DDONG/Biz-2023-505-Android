...
class _ViewBooksPageState extends State<ViewBooksPage> {
  ...
  void searchBooks(String search) {
    ...
    // return 받은 결과를 resultBooks state 변수에 반영하여 List 화면을 다시 그리기
    setState(() {
...
class ViewListPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      // Future<List<Dto>> type 의 데이터들을 사용하여 화면에 보여질 Widget 을 생성하는 도구
      child: FutureBuilder(
        future: resultBooks,
        // FutureBuilder 가 resultBooks 데이터를 사용하여 List 데이터로 변환을 하고, 그 데이터를 다시 ListView.builder 에게 전달하여
        // 실제로 화면에 보여질 List를 생성한다
        builder: (context, snapshot) {
          return ListView.builder(
            itemCount: snapshot.data!.length,
            // ListView.builder 는 리스트의 각 항목(한개 데이터)들을
            // ListTile 에게 전달하여 한개 데이터를 표현할 View 를 만들고, 모아서 리스트로 보여준다
...
class BookItemView extends StatelessWidget {
  ...
  // 함수로 Widget 을 분리한 후 return type 을 Widget 으로 변환하자
