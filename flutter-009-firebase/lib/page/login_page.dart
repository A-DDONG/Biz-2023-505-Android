                      ...
                      onPressed: () async {
                        try {
                          if (_formKey.currentState!.validate()) {
                            ...
                            // main.dart 의 _authUser State 에 로그인한 사용자 정보 업데이트 요청하기
                            widget.updateAuthUser(result.user);
                            // 화면전환, SnackBar 등을 화면에 표현하고자 할때
                            // Don't use BuildContext... 의 경고가 나타나면 아래의 코드를 먼저 실행하도록 추가한다
                            // BuildContext(context)가 아직 완전히 생성되지 않거나 어떤 이유로 context 가 사라질수도 있는데
                            // 사용상 주의하라는 경고
                            // mounted 라는 시스템 변수가 생성되었는지 확인한 후 context 관련된 코드를 실행하라 라는 의미
                            if (!mounted) return;
                      ...
                      onPressed: () async {
                        try {
                          ...
                          if (googleUser == null || googleAuth == null) {
                            ...
                            // 구글이 보내준 인증정보를 사용하여 파이어베이스에 로그인하기
                            // oAuth2 방식의 login
