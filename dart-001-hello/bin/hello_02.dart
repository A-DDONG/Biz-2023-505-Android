...
void main(List<String> args) {
  ...
  // sayeHello() 함수를 호출하면서 3개의 값을 전달한다
  // 이 때 sayHello() 함수에 선언된 3개의 매개변수에 순서대로 값이 저장되어 전달된다
  var result = sayHello("홍길동", 22, "대한민국");
  print(result);

  /// named parameter 방식
  /// 함수를 선언할때 매개변수를 {} 로 감싸주고 값을 전달할때 변수:값 형식으로 전달한다
  /// 위치기반에 비하여 함수에 값이 잘못전달 되는 것을 방지할 수 있다

  result = sayHelloNameParams(age: 33, nation: "우리나라", name: "성춘향");
  print(result);

  /// named parameter 방식의 함수를 선언할때 type 에 null safe(?) 를 지정하면
