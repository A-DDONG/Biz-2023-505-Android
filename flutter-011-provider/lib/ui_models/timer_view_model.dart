...
class TimerViewModel extends ChangeNotifier {
  ...
  /// 이미 생성된 객체를 재활용하는 방법이 있다
  /// static(프로젝트가 시작되면 자동으로 객체 생성)에 비해서 필요한 경우에만 객체를 한번 생성하여
  /// 재활용한다는 의미에서 static 에 비하여 메모리 효율성을 기대할 수 있다
  ...
  Future<int> getTimer() async {
    /// 이 클래스에서 만들어진 _prefs 객체를 private 으로 사용하기 위하여 요구하기
    final SharedPreferences prefs = await _prefs;
    // localStorage(SP)에 TIMER 라는 이름으로 변수가 저장되어 있으면 정수형으로 변환하여 timer 변수에 담아달라
